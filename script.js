...
// Fetch weather report using city name
function getWeatherReport(city) {
  // fetch method fetching the data from base url
  // Add `metric` to parameters to get unit in celcius
  fetch(`${weatherApi.baseUrl}?q=${city}&appid=${weatherApi.key}&units=metric`)
    .then((weather) => {
      return weather.json(); // convert weather data to JSON format
    })
    .then(showWeaterReport); // display weather details to the UI
}
...
// Dynamically change background image based on weather status
...
// Assign classnames to every icons
...
// Append `0` to display when necessary
function addZero(i) {
  if (i < 10) {
    i = "0" + i;
  }
